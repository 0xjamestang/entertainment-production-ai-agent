...
class SnapshotService {
    ...
    async createSnapshot(prompt, includeEntireProjectStructure, includeAllFiles, selectedFiles) {
        try {
            const workspaceRoot = this.getWorkspaceRoot();
            ...
            vscode.window.showErrorMessage('Error creating snapshot: ' + error.message);
        }
    }
    ...
    isExcluded(filePath, patterns) {
        return patterns.some(pattern => new RegExp(pattern.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/\\\*/g, '.*')).test(filePath));
    }
    isIncluded(filePath, patterns) {
    ...
    formatProjectStructure(tree, level = 0) {
        let result = '';
...
class SnapshotDialog {
    constructor() {
        ...
        this.editorListener = vscode.Disposable.from(vscode.window.onDidChangeActiveTextEditor(() => {
    ...
    async show() {
        ...
        this.updateActiveEditors();
        this.panel = vscode.window.createWebviewPanel('snapshotDialog', 'Create Snapshot', vscode.ViewColumn.One, {
            enableScripts: true,
            ...
            <div class="form-group">
                ...
                <div id="fileListContainer" class="file-container" ${this.config.default.default_include_all_files ? 'style="display: none;"' : ''}>
                    ...
                    <div id="fileList" class="file-list">
                        ${relativeFiles.map(file => `<div class="checkbox-label"><input type="checkbox" value="${file}" checked>${file}</div>`).join('')}
                    </div>
                </div>
            </div>
        ...
        return new Promise((resolve) => {
            ...
            this.panel.webview.onDidReceiveMessage(async (message) => {
                switch (message.command) {
                    case 'submit':
                        const selectedFiles = message.selectedFiles.map((file) => vscode.Uri.file(path.join(workspaceRoot, file)));
                        resolve({
